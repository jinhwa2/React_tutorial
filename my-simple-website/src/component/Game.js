...
const Game = () => {
  ...
  // 숫자 맞추려고 시도한 횟수 처음에는 숫자를 맞추려고 시도한적이 없기 때문에 0
  ...
  //<input type="number" value={guess} onChange={handleChange} placeholder='1에서 10사이의 숫자 입력하기'/>
   ...
   return(
    <div>
      ...
      {/*자바스크립트에서 제일 많이쓰는 if문은 삼항 연산자
        여기에표시한 내용 ? true일 때 실행할 내용 : false 일 때 실행할 내용
        true나 false에서 실행할 내용이 많으면 ()괄호로 묶어서 표현
      */}
      { isCorrect ?  (<Link to="/game-twoStep"><button>다음단계로 이동</button></Link>)  
            :     (<button onClick={handleRestart}>재시작버튼</button>)    }

{ isCorrect ?  (<Link to="/game"><button>처음으로이동</button></Link>)  
            :     (<button onClick={handleRestart}>재시작버튼</button>)  }   
